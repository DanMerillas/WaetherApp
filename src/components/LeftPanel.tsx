...
export default function LeftPanel(props: { currentWeather: any; location: string; enableLocation: any; hanlerSearch: any;}) {
        ...
        <>
            {props.currentWeather ? <article className="App-left">
                ...
                <div style={{ display: viewSearhBox ? 'flex' : 'none' }} className="App-weather-location-search-control">
                    <input className="app-input-search" type="text" placeholder="search location" onChange={(value)=>setTextToSearch(value.target.value)} onKeyUp={(event)=>{
                        if (event.key === 'Enter') {
                            handlerSearch()
                        }
                    }}/>
                ...
                <div className='App-weather App-number'>
                    {props.currentWeather.temp ? Math.round(props.currentWeather.temp): Math.round(props.currentWeather.main.temp)}º
                </div>
